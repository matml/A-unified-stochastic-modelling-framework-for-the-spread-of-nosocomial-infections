...
# This code computes the reproduction number of a HCW among patients. Figure 6 left
...
# The heatmap contains 51x51 values E[R^{(j)}(k)], for different values of (gamma_H,eta)
num_eta=precision
num_gammaH=precision

etas=[]
gammaHs=[]

mean_R0_HCW=np.asmatrix(np.zeros((num_eta,num_gammaH)))

for eta_index in range(num_eta):
	print(eta_index)
	eta=0.1+eta_index*(0.9-0.1)/(num_eta-1.0)
	etas=np.append(etas,eta)
	
	for gammaH_index in range(num_gammaH):
		print(gammaH_index)
		gammaH=1.0+gammaH_index*(24.0-1.0)/(num_gammaH-1.0)
		gammaHs=np.append(gammaHs,gammaH)
		...
		mean_R0_HCW[num_eta-1-eta_index,gammaH_index]=mean_j_k

fig = plt.figure()
ax = fig.add_subplot(111)
cax = plt.imshow(mean_R0_HCW, cmap='hot', interpolation='nearest',extent=[gammaHs[0],gammaHs[num_gammaH-1],etas[0],etas[num_eta-1]],aspect="auto")
barra = fig.colorbar(cax)
barra.set_label('$E[R_{(0,1,0)}^{(2)}]$', fontsize=15)

ax.set_xlabel('$\gamma_H$', fontsize=15)
ax.set_ylabel('$\eta$', fontsize=15)

plt.savefig('Figure6_Left.png')
