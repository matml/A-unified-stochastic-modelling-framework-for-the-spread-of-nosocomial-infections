...
# This code computes the reproduction number of a Patient at Room 1. Figure 12 left

# Since patients at room 1 are at compartmental level 1, we set j=1. We are interested in computing R^{(j)}
j=1
...
mean_R0_Patient_Room1=np.asmatrix(np.zeros((num_betaSRs,num_betaDRs)))
...
for betaSR_index in range(num_betaSRs):
	...
	for betaDR_index in range(num_betaDRs):
		...
		while(suma<p_trunc and n<500):
			...
			if suma>p_trunc:
				break
		mean_R0_Patient_Room1[num_betaDRs-1-betaDR_index,betaSR_index]=mean

fig = plt.figure()
ax = fig.add_subplot(111)
cax = plt.imshow(mean_R0_Patient_Room1, cmap='hot', interpolation='nearest',extent=[betaDRs[0],betaDRs[num_betaDRs-1],betaSRs[0],betaSRs[num_betaSRs-1]],aspect="auto")
barra = fig.colorbar(cax)
barra.set_label(r'$E[R_{(1,0,0,0)}^{(1)}]$', fontsize=15)

ax.set_xlabel(r'$\beta_{SR}$', fontsize=15)
ax.set_ylabel(r'$\beta_{DR}$', fontsize=15)

plt.savefig('Figure12_Left.png')
