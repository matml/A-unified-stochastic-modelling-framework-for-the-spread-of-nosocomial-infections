...
# This code computes the reproduction number of a HCW among surfaces. Figure 9 right

# Since HCWs are at compartmental level 2, we set j=2. We are interested in computing R^{(j)}(k)
j=2
...
# Since we are interested in computing the reproduction number R^{(j)}(k) for a HCW among surfaces, we set k=3
k=3
...
# The heatmap contains 26x26 values E[R^{(j)}(k)], for different values of (beta_se,mu)
num_betas_se=precision
num_mus=precision

betas_se=[]
mus=[]

mean_R0_HCW_to_surfaces=np.asmatrix(np.zeros((num_betas_se,num_mus)))

for beta_se_index in range(num_betas_se):
	print(beta_se_index)
	betase=1.5+beta_se_index*(2.5-1.5)/(num_betas_se-1.0)
	betas_se=np.append(betas_se,betase)
	...
	for mu_index in range(num_mus):
		...
		while(suma<p_trunc and n<500):
			...
			if suma>p_trunc:
				break
		mean_R0_HCW_to_surfaces[num_betas_se-1-beta_se_index,mu_index]=mean_j_k

fig = plt.figure()
ax = fig.add_subplot(111)
cax = plt.imshow(mean_R0_HCW_to_surfaces, cmap='hot', interpolation='nearest',extent=[mus[0],mus[num_mus-1],betas_se[0],betas_se[num_betas_se-1]],aspect="auto")
barra = fig.colorbar(cax)
barra.set_label('$E[R_{(0,1,0)}^{(2)}(3)]$', fontsize=15)

ax.set_xlabel('$\mu$', fontsize=15)
ax.set_ylabel(r'$\beta_{se}$', fontsize=15)

plt.savefig('Figure9_Right.png')
