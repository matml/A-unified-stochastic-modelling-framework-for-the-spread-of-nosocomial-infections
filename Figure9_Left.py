...
# This code computes the reproduction number of a HCW among patients. Figure 9 left
...
# Since we are interested in computing the reproduction number R^{(j)}(k) for a HCW among patients, we set k=1
k=1
...
# The heatmap contains 26x26 values E[R^{(j)}(k)], for different values of (beta_sp,mu)
num_betas_sp=precision
num_mus=precision

betas_sp=[]
mus=[]

mean_R0_HCW_to_patients=np.asmatrix(np.zeros((num_betas_sp,num_mus)))

for beta_sp_index in range(num_betas_sp):
	print(beta_sp_index)
	betasp=0.1+beta_sp_index*(0.6-0.1)/(num_betas_sp-1.0)
	betas_sp=np.append(betas_sp,betasp)
	...
	for mu_index in range(num_mus):
		...
		while(suma<p_trunc and n<500):
			...
			if suma>p_trunc:
				break
		mean_R0_HCW_to_patients[num_betas_sp-1-beta_sp_index,mu_index]=mean_j_k

fig = plt.figure()
ax = fig.add_subplot(111)
cax = plt.imshow(mean_R0_HCW_to_patients, cmap='hot', interpolation='nearest',extent=[mus[0],mus[num_mus-1],betas_sp[0],betas_sp[num_betas_sp-1]],aspect="auto")
barra = fig.colorbar(cax)
barra.set_label('$E[R_{(0,1,0)}^{(2)}(1)]$', fontsize=15)

ax.set_xlabel('$\mu$', fontsize=15)
ax.set_ylabel(r'$\beta_{sp}$', fontsize=15)

plt.savefig('Figure9_Left.png')
