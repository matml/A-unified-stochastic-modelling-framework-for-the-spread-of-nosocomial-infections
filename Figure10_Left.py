...
# This code computes the reproduction number of a Surface among patients. Figure 10 left
...
# Since we are interested in computing the reproduction number R^{(j)}(k) for a Surface among patients, we set k=1
k=1
...
# The heatmap contains 26x26 values E[R^{(j)}(k)], for different values of (beta_ep,kappa)
num_betas_ep=precision
num_kappas=precision

betas_ep=[]
kappas=[]

mean_R0_Surface_to_patients=np.asmatrix(np.zeros((num_betas_ep,num_kappas)))

for beta_ep_index in range(num_betas_ep):
	print(beta_ep_index)
	betaep=0.1+beta_ep_index*(0.6-0.1)/(num_betas_ep-1.0)
	betas_ep=np.append(betas_ep,betaep)
	...
	for kappa_index in range(num_kappas):
		...
		while(suma<p_trunc and n<500):
			...
			if suma>p_trunc:
				break
		mean_R0_Surface_to_patients[num_betas_ep-1-beta_ep_index,kappa_index]=mean_j_k

fig = plt.figure()
ax = fig.add_subplot(111)
cax = plt.imshow(mean_R0_Surface_to_patients, cmap='hot', interpolation='nearest',extent=[kappas[0],kappas[num_kappas-1],betas_ep[0],betas_ep[num_betas_ep-1]],aspect="auto")
barra = fig.colorbar(cax)
barra.set_label('$E[R_{(0,0,1)}^{(3)}(1)]$', fontsize=15)

ax.set_xlabel('$\kappa$', fontsize=15)
ax.set_ylabel(r'$\beta_{ep}$', fontsize=15)

plt.savefig('Figure10_Left.png')
