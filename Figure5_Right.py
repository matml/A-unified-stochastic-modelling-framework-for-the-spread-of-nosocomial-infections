...
#				if h!=j:
				...
				for p in range(1,M+1):
					denominator+=(Ns[h-1]-a[h-1])*a[p-1]*Betas[p-1,h-1]
#				else:
#				denominator+=a[h-1]*Gama[h-1]+Gama[h-1]/(1.0-phi)+(Ns[h-1]-a[h-1])*Lambdas[h-1]
#				for p in range(1,M+1):
#					denominator+=(Ns[h-1]-a[h-1])*a[p-1]*Betas[p-1,h-1]
			if n>0:
				if a[k-1]<Ns[k-1]:
				...
				b[Pos_R0(a,Ns,M,j),0]+=Gama[j-1]/denominator#(Gama[j-1]/(1.0-phi))/denominator
...
# This code computes the reproduction number of a patient among Volunteers. Figure 5 right
...
# Since we are interested in computing the reproduction number R^{(j)}(k) for a patient among volunteers, we set k=3
k=3

# Since we can compute probabilities p(R^{(j)}(k)=n), n=0,1,...., we can truncate the distribution so that total mass 0.99 is accumulated
p_trunc=0.99

precision=51

# The heatmap contains 51x51 values E[R^{(j)}(k)], for different values of (delta_C,xi)
...
mean_R0_Patient_to_Volunteers=np.asmatrix(np.zeros((num_xis,num_deltaCs)))
...
for deltaC_index in range(num_deltaCs):
	...
	for xi_index in range(num_xis):
		print(xi_index)
		xi=0.1+xi_index*(0.9-0.1)/(num_xis-1.0)
		...
		mean_R0_Patient_to_Volunteers[num_xis-1-xi_index,num_deltaCs-1-deltaC_index]=mean_j_k

fig = plt.figure()
ax = fig.add_subplot(111)
cax = plt.imshow(mean_R0_Patient_to_Volunteers, cmap='hot', interpolation='nearest',extent=[inv_deltaCs[num_deltaCs-1],inv_deltaCs[0],xis[0],xis[num_xis-1]],aspect="auto")
