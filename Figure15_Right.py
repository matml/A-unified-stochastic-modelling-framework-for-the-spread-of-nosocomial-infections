...
# This code computes the reproduction number of the Peri HCW. Figure 15 right

# Since Peri HCW is at compartmental level 11, we set j=11. We are interested in computing R^{(j)}
j=11
...
# The heatmap contains 21x21 values E[R^{(j)}], for different values of (beta_Peri,mu)
num_bPeris=precision
num_mus=precision

bPeris=[]
mus=[]

mean_R0_Peri=np.asmatrix(np.zeros((num_bPeris,num_mus)))

for bPeri_index in range(num_bPeris):
	print(bPeri_index)
	bPeri=0.05+bPeri_index*(0.1-0.05)/(num_bPeris-1.0)
	bPeris=np.append(bPeris,bPeri)
	...
	for mu_index in range(num_mus):
		...
		while(suma<p_trunc and n<500):
			...
			if suma>p_trunc:
				break
		mean_R0_Peri[num_bPeris-1-bPeri_index,mu_index]=mean

fig = plt.figure()
ax = fig.add_subplot(111)
cax = plt.imshow(mean_R0_Peri, cmap='hot', interpolation='nearest',extent=[mus[0],mus[num_mus-1],bPeris[0],bPeris[num_bPeris-1]],aspect="auto")
barra = fig.colorbar(cax)
barra.set_label(r'$E[\sum_{j=1}^4R_{(0,\dots,0,1)}^{(11)}(j)]$', fontsize=15)

ax.set_xlabel(r'$\mu$', fontsize=15)
ax.set_ylabel(r'$\beta_{Peri}$', fontsize=15)

plt.savefig('Figure15_Right.png')
