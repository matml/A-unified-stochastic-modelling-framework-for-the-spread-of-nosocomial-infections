...
# This code computes the reproduction number of an AP1 HCW (AP1_1). Figure 15 left

# Since AP1 HCW is at compartmental level 5, we set j=5. We are interested in computing R^{(j)}
j=5
...
# The heatmap contains 21x21 values E[R^{(j)}], for different values of (beta_AP1,mu)
num_bAP1s=precision
num_mus=precision
...
mean_R0_AP1=np.asmatrix(np.zeros((num_bAP1s,num_mus)))
...
for bAP1_index in range(num_bAP1s):
	...
	for mu_index in range(num_mus):
		...
		while(suma<p_trunc and n<500):
			...
			if suma>p_trunc:
				break
		mean_R0_AP1[num_bAP1s-1-bAP1_index,mu_index]=mean

fig = plt.figure()
ax = fig.add_subplot(111)
cax = plt.imshow(mean_R0_AP1, cmap='hot', interpolation='nearest',extent=[mus[0],mus[num_mus-1],bAP1s[0],bAP1s[num_bAP1s-1]],aspect="auto")
barra = fig.colorbar(cax)
barra.set_label(r'$E[R_{(0,0,0,0,1,0,\dots,0)}^{(5)}(1)]$', fontsize=15)

ax.set_xlabel(r'$\mu$', fontsize=15)
ax.set_ylabel(r'$\beta_{AP1}$', fontsize=15)

plt.savefig('Figure15_Left.png')
