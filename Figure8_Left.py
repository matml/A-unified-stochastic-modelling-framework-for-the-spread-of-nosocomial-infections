...
# This code computes the reproduction number of a patient among HCWs. Figure 8 left
...
# The heatmap contains 21x21 values E[R^{(j)}(k)], for different values of (beta_ps,gamma'^{-1})
num_betas_ps=precision
...
mean_R0_Patient_to_HCWs=np.asmatrix(np.zeros((num_betas_ps,num_gammaprimes)))

for beta_ps_index in range(num_betas_ps):
	print(beta_ps_index)
	betaps=1.5+beta_ps_index*(2.5-1.5)/(num_betas_ps-1.0)
	betas_ps=np.append(betas_ps,betaps)
	...
	for gammaprime_index in range(num_gammaprimes):
		...
		while(suma<p_trunc and n<500):
			...
			if suma>p_trunc:
				break
		mean_R0_Patient_to_HCWs[num_betas_ps-1-beta_ps_index,gammaprime_index]=mean_j_k

fig = plt.figure()
ax = fig.add_subplot(111)
cax = plt.imshow(mean_R0_Patient_to_HCWs, cmap='hot', interpolation='nearest',extent=[inv_gammaprimes[0],inv_gammaprimes[num_gammaprimes-1],betas_ps[0],betas_ps[num_betas_ps-1]],aspect="auto")
barra = fig.colorbar(cax)
barra.set_label('$E[R_{(1,0,0)}^{(1)}(2)]$', fontsize=15)

ax.set_xlabel('$\gamma^{-1}$', fontsize=15)
ax.set_ylabel(r'$\beta_{ps}$', fontsize=15)

plt.savefig('Figure8_Left.png')
