...
# This code computes the reproduction number of a Patient (Patient P1a). Figure 14, right
...
# The heatmap contains 21x21 values E[R^{(j)}], for different values of (beta_Peri,gamma^{-1})
...
mean_R0_Patient=np.asmatrix(np.zeros((num_bPeris,num_gammas)))
...
for bPeri_index in range(num_bPeris):
	...
	for gamma_index in range(num_gammas):
		...
		while(suma<p_trunc and n<500):
			...
			if suma>p_trunc:
				break
		mean_R0_Patient[num_bPeris-1-bPeri_index,gamma_index]=mean

fig = plt.figure()
ax = fig.add_subplot(111)
cax = plt.imshow(mean_R0_Patient, cmap='hot', interpolation='nearest',extent=[inv_gammas[0],inv_gammas[num_gammas-1],bPeris[0],bPeris[num_bPeris-1]],aspect="auto")
...
plt.savefig('Figure14_Right.png')
